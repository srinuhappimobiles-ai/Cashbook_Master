...
def process_cashbook_ocr(img_np, target_branch):
  """Accurately extracts Denomination and Approval line items strictly."""
  ...
  # Denomination Total (Integer only)
  denom_val = ""
  ...
  # Extract Approval line items (excluding Add Ins)
  approval_amounts = []
  for b in boxes:
    if (
        y_start_approvals < b["y"] < y_end_approvals
        and width * 0.40 <= b["x"] <= width * 0.72
    ):
      val = extract_number(b["text"])
      if val and val > 0 and val not in [2000, 500, 200, 100, 50, 20, 10, 5]:
        approval_amounts.append(val)
  ...
  db["manual_edits"][target_branch]["DENOMINATION"] = denom_val
...
if addins_dump_file:
  ...
  if st.session_state.get("last_addins_file") != current_addins_id:
    try:
      ...
      # Identify branch and amount columns
      ...
      if amt_col is None:
        amt_col = (
            df_addins.columns[1]
            if len(df_addins.columns) > 1
            else df_addins.columns[0]
        )

      # Group amounts by normalized branch name
      addins_dict = {}
      ...
      st.success(
          f"✅ Successfully mapped Addins for **{mapped_count}** stores!"
      )
      st.rerun()
