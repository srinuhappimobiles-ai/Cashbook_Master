...
def process_cashbook_ocr(img_np, target_branch):
    ...
    # 1. Denomination Total (Integer only)
    denom_val = ""
    ...
    # 2. Extract Approval line items (excluding 1449 from Add Ins)
    approval_amounts = []
    for b in boxes:
        if y_start_approvals < b["y"] < y_end_approvals and width * 0.40 <= b["x"] <= width * 0.72:
            val = extract_number(b["text"])
    ...
    db["manual_edits"][target_branch]["AddinGS"] = ""
